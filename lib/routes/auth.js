...
router
  ...
  .put(`/users/:userId/roles/:role`, ensureAuth(), ensureRole(), (req, res, next) => {
    User.updateById(
      req.params.userId,
      {
        $addToSet: {
  ...
  .delete(`/users/:userId`, ensureAuth(), ensureRole(), ({ params }, res, next) => {
    console.log(params);
  ...
  .delete('/users/:userId/roles/:role', ensureAuth(), ensureRole(), ({ params, user }, res, next) => {
    if(user.id !== params.id) {
      ...
      throw new Error('Admin can not delete their own Role');
    }
  });
